...
public class DataAccess {
	...
	public Kuota sortuKuota(Question question, String deskripzioa, double pronostikoa) throws KuotaAlreadyExist {
		System.out.println(">> DataAccess: createKuota=> question= " + question + " dezkripzioa= " + deskripzioa
				+ " pronostikoa=" + pronostikoa);

		Question q = db.find(Question.class, question.getQuestionNumber());
		if (q.DoesKuotaExists(deskripzioa))
			throw new KuotaAlreadyExist("ErrorQueryAlreadyExist!!");
		db.getTransaction().begin();
...
//		if (us.DoesApustuaExists(q)) throw new ApustuaAlreadyExist("ErrorApustuaAlreadyExist");
		...
		if (us.getDiruZorroa() < zenbatekoa)
			throw new DirurikEZ("Ez duzu diru nahikorik");
	...
	public void updateQuestion(Integer ID, String result) throws EmaitzaExist {
		...
		if (q.getResult().compareTo(result) == 0)
			throw new EmaitzaExist("Emaitza gordeta dago jada");
		q.setResult(result);
		db.getTransaction().commit();
	...
	public void erreplikatu(Erabiltzaile user, String posta) throws ErabiltzaileNoExist {
		...
		if (er == null)
			throw new ErabiltzaileNoExist(ResourceBundle.getBundle("Etiquetas").getString("ErNE"));
		db.getTransaction().begin();
		er.addReplikatu(us);
		db.getTransaction().commit();
